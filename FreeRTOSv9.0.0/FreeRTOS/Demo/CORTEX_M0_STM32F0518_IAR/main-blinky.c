...
#define	mainQUEUE_BLINK_TASK_PRIORITY		( tskIDLE_PRIORITY + 1 )
#define mainQUEUE_DATATIME_TASK_PRIORITY    ( tskIDLE_PRIORITY + 1 )
...
void main_blinky( void )
{
	...
	if( xQueue != NULL )
	{
        ...
        int blink_time = 500;
        xTaskCreate( prvBlinkTask, "BLINK LED", configMINIMAL_STACK_SIZE, &blink_time, mainQUEUE_BLINK_TASK_PRIORITY, NULL );
        ...
        xTaskCreate( prvDataTimeTask, "DATA TIME", configMINIMAL_STACK_SIZE*2, &set_time, mainQUEUE_DATATIME_TASK_PRIORITY, NULL );
...
static void prvBlinkTask( void *pvParameters )
{
    unsigned long ulReceivedValue = *(long*)pvParameters;
    const TickType_t xDelay = pdMS_TO_TICKS( ulReceivedValue );
	for( ;; )
	{
        vTaskDelay( xDelay );
...
static void  prvDataTimeTask( void *pvParameters )
{
    struct tm *set_time = (struct tm*)pvParameters;
	...
	for( ;; )
	{
        time_t now;
        now = time(NULL);
        printf("localtime = %s", asctime(localtime(&now)));
        printf("gmtime    = %s\n", asctime(gmtime(&now)));
        vTaskDelay( xDelay );
	}
}
